...
import com.example.cs195tennis.database.Tools;
...
//            tournamentRecord.store();
    ...
    public static void testPredicate() {
        ...
        Tools.print(
                ctx().select()
                        ...
                        .where(GrandSlam.GRANDSLAM.ID.like("%a%").and(MATCHPOINT.ID.eq(1)))
                        .fetch()
        );
        ...
        Tools.print(
                ctx().select()
                        ...
                        .where(Tables.GRANDSLAM.ID.in(select(MATCHPOINT.ID).from(MATCHPOINT)))
                        .fetch()
        );
    }
    ...
    public static void optimisticLocking() throws SQLException {
        ...
        try {
            ...
            MatchPointRecord match1 = dsl.selectFrom(MATCHPOINT).where(MATCHPOINT.SET_NO.eq(1)).fetchOne();
    ...
    public static void resultSets() {
        ...
        DSL.using(connection)
                .select(MATCHPOINT.ID, MATCHPOINT.SET_NO)
                .from(MATCHPOINT)
                .fetch()
                .stream()
