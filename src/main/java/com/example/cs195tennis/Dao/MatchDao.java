...
public class MatchDao {
    ...
    public static Map<String, List<Match>> readAtpMatchToMap() throws FileNotFoundException {
        Multimap<String, Match> tourneyMap = ArrayListMultimap.create();
        ...
        allMatchesCsv.forEach(row -> {
            map.computeIfAbsent(row.get(0) + "-" + row.get(1), k -> new ArrayList<>());

            map.get(row.get(0) + "-" + row.get(1)).add(new Match(row.get(0), row.get(1),row.get(2),row.get(3),row.get(4),row.get(5),row.get(6), row.get(7), row.get(8), row.get(9), row.get(10), row.get(11), row.get(12), row.get(13), row.get(14),
            ...
            ;});
        System.out.println("Map tourney_id to model with all fields");

        map.forEach((k,v) -> System.out.println("\nk: " + k + "\nitems in each bucket: " + v.size()));
