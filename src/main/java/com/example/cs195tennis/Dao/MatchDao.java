...
public class MatchDao {
    ...
    public static Multimap<String, Match> readAtpMatchToMap() throws FileNotFoundException {
        Multimap<String, Match> winnerAndLoserMap = ArrayListMultimap.create();
        ...
        allMatchesCsv.forEach(row -> {
            winnerAndLoserMap.put(row.get(7),
                    new Match(row.get(0), row.get(1), row.get(7), row.get(8), row.get(9), row.get(10), row.get(11), row.get(12), row.get(13), row.get(14),
                            row.get(15),row.get(16), row.get(17), row.get(18), row.get(19), row.get(20), row.get(22), row.get(23)));
            ;});

        System.out.println(" WinnerMap entries " + winnerAndLoserMap.size());

        return winnerAndLoserMap;
    }
